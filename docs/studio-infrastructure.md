...
Remaining acceptance: browser visual QA is blocked by browser-tool connection timeouts. AI credentials/model are not configured, so real AI responses and spending remain disabled. Test an actual provider-originating message and approve an outgoing reply deliberately; validate signed webhook delivery against the eventual customer's owned receiver; configure AI and check grounding, handoff and budget boundaries. These steps require no customer enrollment or fabricated payment. The generic CRM contract still needs the customer's field mapping and an adapter for its specific system.
