...
let authResult={tenant:randomUUID(),actor:randomUUID()},calls=[];const filters=[];
const db={rpc:async(name,args)=>{calls.push({name,args});return name==='soulvd_crm_auth'?{data:authResult,error:null}:{data:{allowed:true,id:'job',message_id:'message'},error:null};},from:table=>{
