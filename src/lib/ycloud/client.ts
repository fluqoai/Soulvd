...
export async function ycloud<T>(path: string, body?: unknown): Promise<T> {
  ...
  if (!writePath && !(body === undefined && readPath))
    throw new YCloudError("INVALID_PROVIDER_PATH");
  ...
  try {
    ...
    throw new YCloudError("PROVIDER_CONNECTION_UNCERTAIN", body !== undefined);
  }
  let result;
  try {
    result = await response.json();
  } catch {
    throw new YCloudError("PROVIDER_RESPONSE_UNCERTAIN", body !== undefined);
  }
...
export async function providerTemplates(wabaId: string) {
  ...
  for (let page = 1; page <= 10; page++) {
    const result = await ycloud<{ items: ProviderTemplate[] }>(
      `/whatsapp/templates?filter.wabaId=${encodeURIComponent(wabaId)}&limit=100&page=${page}`,
    );
    if (!Array.isArray(result.items))
      throw new YCloudError("PROVIDER_INVALID_RESPONSE");
