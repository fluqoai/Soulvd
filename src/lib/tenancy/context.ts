...
export const currentMerchant = cache(async function currentMerchant() {
...
export const tenantContext = cache(
  async function tenantContext(): Promise<TenantContext | null> {
    ...
    let membership = db
      ...
      .eq("user_id", user.id);
    if (selected && /^[a-f0-9-]{36}$/i.test(selected))
      membership = membership.eq("tenant_id", selected);
    let { data, error } = await membership
      .order("tenant_id")
      .limit(1)
      .maybeSingle();
...
export const tenantUsage = cache(async function tenantUsage() {
  ...
  if (
    ...
    throw new Error("تعذر تحميل الاستهلاك.");
  ...
  return {
    ...
    isActive:
      s.status === "active" &&
      observedAt >= Date.parse(s.period_start) &&
      observedAt < Date.parse(s.period_end),
    dismissedResources:
      dismissals.data?.map((row) => row.resource as string) ?? [],
    usage: {
      conversations: usage.data?.conversations_used ?? 0,
      seats: (seats.count ?? 0) + (invitations.count ?? 0),
