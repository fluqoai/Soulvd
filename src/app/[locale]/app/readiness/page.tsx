...
export default async function ReadinessPage({
  ...
  const rows = [
    ...
    },
    {
      ...
      href: "/app/whatsapp",
      detail: inbound
        ? "وصلت رسالة من رقم الاختبار إلى Soulvd ضمن نافذة الفحص."
        : phone
    ...
    },
    {
      ...
      href: "/app/whatsapp",
      detail: delivered
        ? "ورد تأكيد بتسليم رد من Soulvd إلى رقم الاختبار ضمن نافذة الفحص."
        : "افتح المحادثات وأرسل ردًا إلى الرقم نفسه، ثم انتظر تأكيد التسليم.",
    },
    {
      title: "تشغيل مسار أتمتة",
      done: Boolean(runs.data?.length),
      href: "/app/automations",
      detail: runs.data?.length
        ? "أنتجت الأتمتة مسودة أو إرسالًا أو تحويلًا للموظف لرسالة ضمن الجولة. افتح الأتمتة لمراجعة النتيجة."
    ...
    },
    {
      ...
      detail:
        (templates.count ?? 0) > 0
          ? "يوجد قالب معتمد في مساحة العمل، ويمكن استخدامه لاختبار التسليم."
    ...
    },
    {
      ...
      href: "/app/whatsapp",
      detail: templateSent
        ? "ورد تأكيد بتسليم رسالة قالب إلى رقم الاختبار ضمن نافذة الفحص."
    ...
    },
    {
      ...
      detail: deliveries.data?.length
        ? "استقبلت وجهة التكامل حدثًا مرتبطًا برسالة في الجولة وأعادت استجابة نجاح. راجع النتيجة داخل نظامك أيضًا."
        : plan.api_enabled
          ? "يتطلب تكاملًا مفعّلًا ووجهة اختبار تخصك. تحقق أيضًا من توقيع الحدث في النظام المستقبل."
          : "يتطلب باقة النمو وتكاملًا مفعّلًا. يتم التفعيل بعد مراجعة الطلب وتأكيد التحويل.",
    },
  ];
      ...
      <aside className="flex items-start gap-3 rounded-2xl border border-sage-100 bg-white p-5">
        ...
        <div className="space-y-2 text-sm leading-7 text-ink-600">
          <p>
            ...
            يثبت صحة ربط حقوله أو أمان التوقيع؛ اختبرهما في النظام المستقبل.
          </p>
          <p>
            يتاح اختبار الذكاء الاصطناعي لاحقًا بعد تفعيل الخدمة وحصتها. نتائج
            المراسلة تخص الرقم والفترة المحددين، بينما تعكس حالة الاشتراك والرقم
            والقوالب إعدادات المساحة الحالية.
          </p>
        </div>
      </aside>
    </div>
