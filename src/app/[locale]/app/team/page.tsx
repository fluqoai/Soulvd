...
import { PendingInvitations } from '@/components/billing/PendingInvitations';
...
export default async function TeamPage() {
  ...
  const { data: invitations, error: inviteError } = context.role === 'agent' ? { data: [], error: null } : await db.from('tenant_invitations').select('id,email,role,expires_at,email_sent_at').eq('tenant_id', context.tenantId).eq('status', 'pending').gt('expires_at', new Date().toISOString()).order('expires_at');
  if (inviteError) throw new Error('تعذر تحميل الدعوات.');
  return <div className="space-y-6"><h1 className="text-3xl font-bold">الفريق</h1><p>عدد أعضاء الفريق: {members?.length ?? 0}. يُحسب صاحب مساحة العمل ضمن المقاعد، وتُحجز الدعوات غير المنتهية ضمن الحد.</p>
    ...
    {context.role !== 'agent' && <PendingInvitations invitations={invitations ?? []} />}
  </div>;
}
