    ...
    <div>
      ...
      <DataTable rows={items} rowKey={(r) => r.id} editHref={(r) => `/admin/partners/${r.id}`} emptyMessage="No partners yet."
        columns={[
          ...
          { key: 'url', header: 'Website', cell: (r) => r.url ? <a href={r.url} target="_blank" rel="noreferrer" className="text-xs text-sage-300 hover:underline truncate max-w-[240px] inline-block align-middle">{r.url}</a> : <span className="text-ink-500">—</span> },
