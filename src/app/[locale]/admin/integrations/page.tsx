  ...
  return (
    <div>
      <PageHeader title="Integrations" description="The integrations grid on the home page and the /services page." actions={
        <ButtonLink href={`/${locale}/admin/integrations/new`} size="sm" variant="primary"><Plus className="size-4" />New</ButtonLink>
      } />
      <DataTable rows={items} rowKey={(r) => r.id} editHref={(r) => `/${locale}/admin/integrations/${r.id}`} emptyMessage="No integrations yet."
        columns={[
