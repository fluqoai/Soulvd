...
export default async function Page({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div>
      <PageHeader title="التكاملات" description="The integrations grid on the home page و the /services page." actions={
      ...
      <DataTable rows={items} rowKey={(r) => r.id} editHref={(r) => `/admin/integrations/${r.id}`} emptyMessage="لا توجد تكاملات بعد."
        columns={[
          ...
          { key: 'logo', header: 'الشعار', width: '80px', cell: (r) => r.logo_url ? <div className="relative h-8 w-16"><Image src={r.logo_url} alt={r.name} fill sizes="64px" className="object-contain" /></div> : <div className="size-8 rounded bg-sage-50 grid place-items-center text-xs text-ink-600">{r.name.slice(0, 2)}</div> },
          { key: 'name', header: 'الاسم', cell: (r) => <span className="font-medium">{r.name}</span> },
          { key: 'category', header: 'الفئة', width: '120px', cell: (r) => r.category ? <span className="px-2 py-0.5 rounded bg-sage-50 text-xs">{r.category}</span> : '—' },
