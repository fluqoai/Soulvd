...
export default function QuotesPage() {
  return (
    <div>
      <PageHeader title="Quotes" description="Generated price quotes — depends on the templates engine." />
      <div className="rounded-2xl border-2 border-dashed border-ink-900/10 bg-paper p-10 text-center max-w-2xl">
        ...
        <p className="text-base text-ink-600 max-w-md mx-auto">
          ...
          create عرض سعر (price quotes) the same way as invoices.
        </p>
        <div className="mt-6 flex items-center justify-center gap-3">
          <Link href="/admin/templates" className="text-sm text-sage-700 hover:underline font-medium">Go to Templates →</Link>
          <Link href="/admin/clients" className="text-sm text-ink-600 hover:underline">Manage clients</Link>
        </div>
      </div>
    </div>
  );
