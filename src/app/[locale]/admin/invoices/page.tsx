...
// Invoices — waiting on the templates engine. Shows clear next steps.
...
export default function InvoicesPage() {
  return (
    <div>
      <PageHeader
        title="الفواتير"
        description="الفواتير المُولّدة — تعمل مع محرك القوالب. ارفع قالباً أولاً ثم ابدأ الإنشاء."
      />
      <div className="rounded-2xl border-2 border-dashed border-ink-900/10 bg-paper p-10 text-center max-w-2xl">
        ...
        <p className="text-base text-ink-600 max-w-md mx-auto">
          ...
          {' '}يحتوي على متغيرات
          {' '}<code className="text-xs bg-linen-100 px-1.5 py-0.5 rounded">{'{{...}}'}</code>
          ، ستتمكن من إنشاء الفواتير هنا: اختر العميل → عبئ القيم → ولّد
          {' '}<code className="text-xs">.docx</code> + PDF → شارك الرابط.
        </p>
